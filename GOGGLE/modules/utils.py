"""Reference:
[1] https://github.com/tennisonliu/GOGGLE/blob/main/src/goggle/model/GraphInputProcessor.py
[2] https://github.com/tennisonliu/GOGGLE/blob/main/src/goggle/model/LearnedGraph.py
[3] https://github.com/tennisonliu/GOGGLE/blob/main/src/goggle/model/RGCNConv.py
"""
#%%
import numpy as np
import random
...
class GraphInputProcessorHomo(nn.Module):
    def __init__(self, config, device):
        ...
        self.device = device
        self.het_encoding = config["het_encoding"]

        if config["het_encoding"]:
    ...
    def forward(self, z, adj):
        ...
        g = dgl.graph((torch.tensor(edge_index[0]), torch.tensor(edge_index[1])))
...
class GraphInputProcessorHet(nn.Module):
    def __init__(self, config, n_edge_types, device):
        ...
        for _ in range(config["input_dim"]):
            self.embedding_functions.append(
                nn.Sequential(
                    nn.Linear(feat_dim, config["decoder_dim"]), 
                    nn.Tanh()
                ).to(device)
            )
