...
class CTABGANSynthesizer:
    def __init__(self, config, data_prep, type={}, device='cpu'):                 
        self.random_dim = config['latent_dim']
        self.class_dim = config['class_dim']
        self.num_channels = config['num_channels']
        self.dside = None
        self.gside = None
        self.l2scale = config['weight_decay']
        ...
        target_index=None
        if type:
            self.problem_type = list(type.keys())[0]
            if self.problem_type:
                target_index = data_prep.df.columns.get_loc(type[self.problem_type])

        self.transformer = DataTransformer(
            train_data=data_prep.df,
            categorical_list=data_prep.column_types["categorical"],
            mixed_dict=data_prep.column_types["mixed"],
            general_list=data_prep.column_types["general"],
            non_categorical_list=data_prep.column_types["non_categorical"])
        
        self.transformer.fit() 
        self.train_data = self.transformer.transform(data_prep.df.values)
        self.data_sampler = Sampler(self.train_data, self.transformer.output_info)
        data_dim = self.transformer.output_dim
        self.cond_generator = Cond(self.train_data, self.transformer.output_info)
        ...
        self.discriminator = Discriminator(self.dside, layers_D).to(self.device)
        ...
        if target_index != None:
            self.st_ed= get_st_ed(target_index,self.transformer.output_info)
            self.classifier = Classifier(data_dim,self.class_dim,self.st_ed).to(self.device)
    ...
    def fit(self):
        ...
        for epoch in range(self.epochs):
            ...
            for id_ in tqdm(range(steps_per_epoch), desc="inner loop"):
                loss_ = []
                ...
                pen = calc_gradient_penalty_slerp(self.discriminator, real_cat, fake_cat,  self.Dtransformer , self.device)

                pen.backward()
            
                self.optimizerD.step()
                
                loss_d = d_real + d_fake + pen
                ...
                if self.problem_type:
                    ...
                    if (self.st_ed[1] - self.st_ed[0])==1:
                        c_loss= SmoothL1Loss()
                    ...
                    elif (self.st_ed[1] - self.st_ed[0])==2:
                        c_loss = BCELoss()
                        real_label = real_label.type_as(real_pre)
                        fake_label = fake_label.type_as(fake_pre)
    ...
    def sample(self, n):
        ...
        while len(result) < n:
            ...
            res,resample = self.transformer.inverse_transform(data_resample, check)
